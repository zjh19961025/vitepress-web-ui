import { testUtils, stringUtils } from "@hua5/hua5-utils"
...
export function addStyle(customStyle: string | object, target = 'object') {
  ...
  if (testUtils.isEmpty(customStyle) || (typeof (customStyle) === 'object' && target === 'object') ||
  ...
  if (target === 'object') {
    ...
    customStyle = stringUtils.trim(customStyle as string)
    // 根据";"将字符串转为数组形式
    ...
    for (let i = 0; i < styleArray.length; i++) {
      ...
      if (styleArray[i]) {
        ...
        style[stringUtils.trim(item[0])] = stringUtils.trim(item[1])
      }
    }
    return style
  }
